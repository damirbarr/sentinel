...
function AtomNucleus({ decisionColor, n }: { decisionColor: string; n: number }) {
  const meshRef = useRef<THREE.Mesh>(null)
  const color = useMemo(() => new THREE.Color(decisionColor), [decisionColor])
  const stabilityFactor = Math.min(n / 8, 1)

  useFrame(({ clock }) => {
    if (!meshRef.current) return
    const t = clock.getElapsedTime()
    const mat = meshRef.current.material as THREE.MeshStandardMaterial
    mat.emissiveIntensity = 0.8 + 0.5 * Math.sin(t * (1.5 + stabilityFactor * 2.5))
    if (stabilityFactor > 0.4) {
      const s = 1 + 0.03 * stabilityFactor * Math.sin(t * 3)
  ...
  return (
    <group>
      ...
      <pointLight color={decisionColor} intensity={1.2 + 0.6 * stabilityFactor} distance={4} />
    </group>
  )
}
...
function ElectronOrbit({ nodeId, color, stabilityFactor, onHover, hoverInfo }: ElectronOrbitProps) {
  const groupRef = useRef<THREE.Group>(null)
  const electronRef = useRef<THREE.Mesh>(null)
  const ringMatRef = useRef<THREE.MeshStandardMaterial>(null)
  const electronMatRef = useRef<THREE.MeshStandardMaterial>(null)
  ...
  const baseRx = ((h % 314) / 100) - Math.PI / 2      // initial X tilt
  const baseRy = ((h >> 4 & 0xff) / 255) * Math.PI    // initial Y tilt
  ...
  useFrame(({ clock }) => {
    ...
    const progress = easeOutCubic(Math.min(age / 0.5, 1))

    if (groupRef.current) {
      groupRef.current.scale.setScalar(progress)
      const drift = stabilityFactor * t * 0.15
    ...
    if (ringMatRef.current) ringMatRef.current.opacity = progress * 0.35
    if (electronMatRef.current) {
      electronMatRef.current.opacity = progress
      electronMatRef.current.emissiveIntensity = progress * 1.4
    }
  })
...
interface AtomNode { id: string; color: string; hoverInfo: HoverInfo }
...
function AtomScene({ decision, activeConstraints, reasonCodes, onHover }: AtomSceneProps) {
  ...
  ], [activeConstraints, syntheticNodes])
  ...
  return (
    <>
      ...
      <AtomNucleus decisionColor={decisionColor} n={n} />
...
export default function AtomCanvas({ decision, reasonCodes, speedKmh, activeConstraints, fullscreen, autoRotate = true }: BrainCanvasProps) {
    ...
    <div style={{ position: 'relative', height: fullscreen ? '100vh' : '260px', userSelect: 'none' }}>
      ...
      {hovered && (
        <div style={{
          ...
          boxShadow: `0 0 12px ${hovered.color}30`,
        }}>
